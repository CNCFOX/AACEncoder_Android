/*
  ...
  Copyright (C) 2014 HSAE yangzhouDep turkeyzhu<turkeyzhu@gmail.com>
  ...
  version : v1.0.1
  author turkeyzhu<turkeyzhu@gmail.com>
...
*/
uint64_t fa_aacenc_init(int sample_rate, int bit_rate, int chn_num,
                         int mpeg_version, int aac_objtype, int lfe_enable,
                         int band_width,
                         int speed_level);

void fa_aacenc_uninit(uint64_t handle);

//WARN: the inlen must be 1024*chn_num*2 (2 means 2 bytes for per sample, so your sample should be 16 bits short type), 
void fa_aacenc_encode(uint64_t handle, unsigned char *buf_in, int inlen, unsigned char *buf_out, int *outlen);
